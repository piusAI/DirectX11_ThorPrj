...
ModelClass::ModelClass() : m_vertexBuffer(0), m_indexBuffer(0), m_instanceBuffer(0), m_model(0),
m_textureCount(0), m_normalCount(0), m_faceCount(0), m_TransformComp(0), m_Texture(0)
{
...
ModelClass::ModelClass(float Tx, float Ty, float Tz) : m_vertexBuffer(0), m_indexBuffer(0), m_instanceBuffer(0), m_model(0),
m_textureCount(0), m_normalCount(0), m_faceCount(0), m_TransformComp(0), m_Texture(0)
{
	m_TransformComp = new TransformComponent();


	m_TransformComp->Translate.x = Tx;
	m_TransformComp->Translate.y = Ty;
	m_TransformComp->Translate.z = Tz;
}

void ModelClass::ModelInstance()
{
	if (instanceNum <= 0)
	{
		OutputDebugStringW(L"0개보다 낮을 수 없음");
		return;
	}
	instances.resize(instanceNum);
}

void ModelClass::SetInstancePosition(int j, float x, float y, float z)
{
	if (j>=(int)instances.size() ||j<0)
	{
		OutputDebugString(L" isntance error!");

		return;
	} //넘지않음
		instances[j].position.x = x;//x위치
		instances[j].position.y = y;//y위치
		instances[j].position.z = z;//z위치
}


ModelClass::ModelClass(float Tx, float Ty, float Tz, float Rx, float Ry, float Rz) : m_vertexBuffer(0), m_indexBuffer(0), m_model(0),
m_textureCount(0), m_normalCount(0), m_faceCount(0), m_TransformComp(0)
{
	m_TransformComp = new TransformComponent();
	...
	m_TransformComp->Rotation.x = Rx;
	m_TransformComp->Rotation.y = Ry;
	m_TransformComp->Rotation.z = Rz;

}



ModelClass::ModelClass(float Tx, float Ty, float Tz, float Rx, float Ry, float Rz, float Sx, float Sy , float Sz) : m_vertexBuffer(0), m_indexBuffer(0), m_model(0),
m_textureCount(0), m_normalCount(0), m_faceCount(0), m_TransformComp(0)
{
	...
	m_TransformComp->Scale.x = Sx;
	m_TransformComp->Scale.y = Sy;
	m_TransformComp->Scale.z = Sz;
...
void ModelClass::SetTranslate(float Tx, float Ty, float Tz)
{
	m_TransformComp->Translate = XMFLOAT3(Tx, Ty, Tz);
	m_TransformComp->InitTransform();
}
...
bool ModelClass::Initialize(ID3D11Device* device, const WCHAR* modelFilename, const WCHAR* textureFilename)
{
	...
	result = m_TransformComp->InitTransform();
	if (!result) return false;
...
void ModelClass::Tick(float deltaTime,XMFLOAT3 translate, float PosSpeed = 1, float RotSpeed=1)
{

		m_TransformComp->MoveToTarget(deltaTime, translate, PosSpeed, RotSpeed);
...
const XMMATRIX& ModelClass::GetTransformMatrix()
{
	return m_TransformComp->RSTMatrix;
...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	// set Instance position 로드 여기서 들어갈그임
	...
	instanceBufferDesc.ByteWidth = sizeof(InstanceType) * instanceNum;
...
void ModelClass::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	*/
	...
	// Set an array of pointers to vertex and instance buffers.

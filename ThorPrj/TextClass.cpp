	...
	int screenWidth, int screenHeight, XMMATRIX baseViewMatrix)
{
	...
	if (!result)
	{
		MessageBox(hwnd, L"font object. 초기화 불가!", L"Error", MB_OK);
		return false;
	}
	...
	if (!result)
	{
		MessageBox(hwnd, L"font shader object 초기화 불가!", L"Error", MB_OK);
		return false;
	}
...
bool TextClass::SetCPU(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, cpuString, 20, 34, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPolygonNum(int ObjectNum, ID3D11DeviceContext* deviceContext)
{
	char tempString[64];
	char PolygonString[64];
	bool result;
	...
	strcpy_s(PolygonString, "Polygon Num = ");
	strcat_s(PolygonString, tempString);

	result = UpdateSentence(m_sentence3, PolygonString, 20, 48, 0.4f, 0.9f, 0.8f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetObjectsNum(int ObjectNum, ID3D11DeviceContext* deviceContext)
{
	char tempString[64];
	...
	strcpy_s(ObjectString, "Object Num = ");
	strcat_s(ObjectString, tempString);

	result = UpdateSentence(m_sentence4, ObjectString, 20, 62, 0.4f, 0.9f, 0.8f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetResolutionNum(int width, int height, ID3D11DeviceContext* deviceContext)
{

	char widthString[64];
	char heightString[64];
	...
	result = UpdateSentence(m_sentence5, ResolutionString, 20, 76, 0.9f, 0.4f, 0.6f, deviceContext);
	if (!result)
	{
		return false;
	}
